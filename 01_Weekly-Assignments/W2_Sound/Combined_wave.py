...
def plot_wave_at_time(frame, pp):
    ...
    for i in range(len(arr)):  # update the position of every point
        ...
        gap = R * np.cos(k * x - w * frame) + (1 - R) * np.cos(k * x + w * frame)
        arr[i][0] += gap
        arr[i][1] += gap
...
if __name__ == '__main__':
    ...
    plt.show(block=True)
    ani.save('Combined_wave.gif', writer='pillow')
    ...
    # plt.savefig('Combined_wave_at_time_{}'.format(t))
