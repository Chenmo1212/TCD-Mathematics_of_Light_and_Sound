...
def plot_wave_at_time(frame, pp):
    ...
    for i in range(len(arr)):
        ...
        arr[i][1] = arr[i][1] + R * np.cos(k * x - w * frame) + (1 - R) * np.cos(k * x + w * frame)
...
if __name__ == '__main__':
    ...
    # ==========================  display the transverse_wave animation =====================

    # set up wave parameters
    wave_frequency = 100  # frequency
    ...
    plt.show(block=True)
    ani.save('Transverse.gif', writer='pillow')

    # ===================  display the transverse_wave at a specific time i =================
    # need to comment out the animation part above

    # t = 3
    # plot_wave_at_time(t, points_pos)
    # plt.savefig('Transverse_wave_at_time_{}'.format(t))
